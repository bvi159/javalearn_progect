...
//            double change = !stock.containsKey("change") ? 0 : ((double) stock.get("change"));
            // --- ват это ИИ придумал
            double open = !stock.containsKey("open") ? 0 : ((Number) stock.get("open")).doubleValue();
            double last = !stock.containsKey("last") ? 0 : ((Number) stock.get("last")).doubleValue();
            double change = !stock.containsKey("change") ? 0 : ((Number) stock.get("change")).doubleValue();
    ...
    public static void sort(List<Stock> list) {
        list.sort(Comparator
                .comparing((Stock stock) -> (String) stock.get("name")) // 1.1. По имени в алфавитном порядке
                .thenComparing(stock -> {
                    Date date = (Date) stock.get("date");
                    ...
                    return cal.getTime();
                }, Comparator.reverseOrder()) // 1.2. По дате (сверху самые новые)
                .thenComparing(stock ->
                                stock.containsKey("change") ? (double) stock.get("change") : 0,
                        Comparator.reverseOrder() // По прибыли (от положительных к отрицательным)
                )
        );
    }
