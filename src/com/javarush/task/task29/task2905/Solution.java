...
Посмотри внимательно на код. Я вижу две проблемы:

Ошибка в цикле создания сообщений - i-- вместо i++ приведет к бесконечному циклу и выходу
за пределы int

Неправильное использование drainTo - метод вызывается с двумя параметрами,
но первым параметром должна быть коллекция для приема данных, а не очередь
...
//                            persistData(batch);
//                            batch.clear();
                            // исправлено: передаем batch первым параметром, а не messageQueue
                            messageQueue.drainTo(batch, MAX_BATCH_SIZE);
                            if (!batch.isEmpty()) { // добавляем проверку на пустоту
                                persistData(batch);
                                batch.clear();
                            }
                            Thread.sleep(1);
