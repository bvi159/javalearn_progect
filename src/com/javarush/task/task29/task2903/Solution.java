...
Требования:
•	Переименуй константу random в соответствии с Naming and Code Convention.
•	Объект, возвращаемый методом getRunnable(), должен быть экземпляром анонимного класса.
•	Метод run() внутри метода getRunnable() должен вызывать у concurrentMap метод, вставляющий пару (randomInt, text), если в concurrentMap еще нет пары со значением ключа randomInt.
•	Метод run() класса, возвращаемого методом getRunnable(), должен выводить текст на экран.
    ...
    public static Runnable getRunnable(final int i, final ConcurrentMap<Integer, String> concurrentMap) {
        return new Runnable() {
            ...
            public void run() {
                ...
                // Instead of setting it to null, call concurrentMap.someMethod(randomInt, text)
//                String previousEntry = null;
                // ИИ Используем putIfAbsent для вставки только если ключ отсутствует
