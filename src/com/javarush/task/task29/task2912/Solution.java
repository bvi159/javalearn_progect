...
Требования:
•	Класс AbstractLogger должен быть публичным и абстрактным.
•	Класс AbstractLogger должен поддерживать интерфейс Logger.
•	Необходимо поднять поля level и next в класс AbstractLogger.
•	Необходимо поднять методы setNext(Logger) и inform(String, int) в класс AbstractLogger.
•	Вывод программы должен остаться неизменным.
